...
import { loanProgramSlugByTitle } from "@/lib/loan-program-pages";

type LoanProgram = (typeof loanPrograms)[number];

export function LoanCard({ program }: { program: LoanProgram }) {
  const Icon = program.icon;
  const publicSlug = loanProgramSlugByTitle.get(program.title);
  const href = publicSlug ? `/loan-programs/${publicSlug}` : `/mortgage-programs/${program.slug}`;
  ...
  return (
    <article className="group rounded-sm border border-navy/10 bg-white p-7 shadow-[0_14px_40px_rgba(13,27,42,0.05)] transition duration-300 hover:-translate-y-1 hover:border-gold/50 hover:shadow-[0_20px_50px_rgba(13,27,42,0.1)]">
      ...
      <Link href={href} className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-navy transition group-hover:text-gold">
